...
def attach_signature(bol_id: int, signature_data: str, signed_by: str) -> dict:
    ...
    try:
        ...
        # Regenerate PDF with signature included
        try:
            # Generate PDF with signature rendered inline
            signed_pdf = generate_bol_pdf(
                bol,
                return_bytes=True,
                signature_data=signature_data,
                signed_by=signed_by,
                signed_at=bol.signed_at
            )
            ...
            logger.info(f"Signed PDF generated for BOL {bol.bol_number}")
